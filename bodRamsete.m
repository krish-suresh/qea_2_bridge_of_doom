...
% Drive robot
    ...
    target_vel = double(subs(speed, u, cur_dist)*cur_vel);
    target_omega = double(subs(omega, u, cur_dist)*cur_vel);
    
    error = target_pose - pose;
    error(3) = normDelta(error(3));
    error = [(R_fun(pi/2 - pose(3))*error(1,1:2)')', error(3)];
    k = 2*ram_zeta*sqrt(target_omega^2 + ram_b*target_vel^2)

%     applied_v = target_vel*cos(error(3))+k*(cos(pose(3))*error(1)+sin(pose(3)*error(2)));
%     applied_w = target_omega+k*error(3) + ram_b*target_vel*sinc(error(3))*(cos(pose(3)*error(2)-sin(pose(3)*error(1))));
    applied_v = target_vel*cos(error(3))+k*error(1);
    applied_w = target_omega+k*error(3) + ram_b*target_vel*sinc(error(3))*error(2);

    vL = double(applied_v - d/2*applied_w);
    vR = double(applied_v + d/2*applied_w);
    msg = rosmessage(pub);
    msg.Data = [vL, vR];
    send(pub, msg);
% Read odo
